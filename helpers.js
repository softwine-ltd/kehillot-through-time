...
// Creates the noUiSlider timeline. Always `direction: 'ltr'`, deliberately NOT matching the
// page's text direction: the timeline reads oldest-on-the-left, newest-on-the-right in every
// language, Hebrew included (unlike prose, which does flip).
//
// Don't pin the timeline element's own CSS `direction` or re-create the slider to "fix" its
// layout. noUiSlider reads two things once, at creation, that have to agree: document.dir (which
// sets the offset it translates .noUi-origin by) and the target's computed `direction` (which
// picks the noUi-txt-dir-ltr/-rtl class that anchors the handle to one side of that origin).
// Left to inherit, they always agree; pinning the element to ltr on a Hebrew page made them
// disagree and drew the handle at the far end of the track from its actual value. The page
// widening this code once tried to work around is the origin box overflowing past the page's
// scrollable edge; `.timeline-section { overflow-x: clip }` in styles.css handles that.
